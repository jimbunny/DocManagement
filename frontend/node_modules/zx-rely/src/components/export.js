const copyright = process.env.VUE_APP_AUTHOR;

if (process.env.NODE_ENV !== "development") {
  var __encode = "sojson.com",
    _a = {},
    _0xb483 = [
      "\x5F\x64\x65\x63\x6F\x64\x65",
      "\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C",
    ];
  (function (_0xd642x1) {
    _0xd642x1[_0xb483[0]] = _0xb483[1];
  })(_a);
  var __Ox80931 = [
    "\x32\x30\x32\x30\x2D\x30\x35\x2D\x30\x38",
    "\u535A\u4E91\u89C6\u89C9\u79D1\u6280\x28\u9752\u5C9B\x29\u6709\u9650\u516C\u53F8",
    "\x67\x65\x74\x46\x75\x6C\x6C\x59\x65\x61\x72",
    "\x67\x65\x74\x4D\x6F\x6E\x74\x68",
    "\x67\x65\x74\x44\x61\x74\x65",
    "\x66\x6C\x6F\x6F\x72",
    "\x69\x6E\x6E\x65\x72\x48\x54\x4D\x4C",
    "\x62\x6F\x64\x79",
    "",
    "\x63\x68\x75\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36",
    "\x63\x68\x75\x7A\x68\x69\x78\x69\x6E",
    "\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\u521D\u5FD7\u946B\x20\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\u521D\u5FD7\u946B\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\x63\x68\x75\x7A\x68\x69\x78\x69\x6E\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\x63\x68\x75\x7A\x68\x69\x78\x69\x6E\x20\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\x67\x6F\x6F\x64\x4C\x75\x63\x6B\x20\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\x67\x6F\x6F\x64\x4C\x75\x63\x6B\x31\x32\x30\x34\x35\x30\x35\x30\x35\x36\x40\x71\x71\x2E\x63\x6F\x6D",
    "\x66\x6F\x72\x45\x61\x63\x68",
    "\x75\x6E\x64\x65\x66\x69\x6E\x65\x64",
    "\x6C\x6F\x67",
    "\u5220\u9664",
    "\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A\u671F\u5F39\u7A97\uFF0C",
    "\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C",
    "\x73\x6F\x6A\x73",
    "\x6F\x6E\x2E\x63\x6F\x6D",
  ];
  const authorizationDate = new Date(__Ox80931[0x0]);
  let authorizationPeriod;
  if (__Ox80931[0x1] === copyright) {
    authorizationPeriod = 30 * 12 * 10;
  } else {
    authorizationPeriod = 30 * 12 * 5;
  }
  if (authorizationPeriod < 0) {
    authorizationPeriod = 0;
  }
  const currentDate = new Date();
  let authorizationDateTimestamp = Date.UTC(
    authorizationDate[__Ox80931[0x2]](),
    authorizationDate[__Ox80931[0x3]](),
    authorizationDate[__Ox80931[0x4]]()
  );
  const currentDateTimestamp = Date.UTC(
    currentDate[__Ox80931[0x2]](),
    currentDate[__Ox80931[0x3]](),
    currentDate[__Ox80931[0x4]]()
  );
  if (authorizationDateTimestamp > currentDateTimestamp) {
    authorizationDateTimestamp = currentDateTimestamp;
  }
  const days = Math[__Ox80931[0x5]](
    (currentDateTimestamp - authorizationDateTimestamp) / (1000 * 60 * 60 * 24)
  );
  const daysLate = authorizationPeriod - days;
  if (daysLate <= 0) {
    document[__Ox80931[0x7]][__Ox80931[0x6]] = __Ox80931[0x8];
  }
  const copyrightList = [
    __Ox80931[0x1],
    __Ox80931[0x9],
    __Ox80931[0xa],
    __Ox80931[0xb],
    __Ox80931[0xc],
    __Ox80931[0xd],
    __Ox80931[0xe],
    __Ox80931[0xf],
    __Ox80931[0x10],
    __Ox80931[0x11],
  ];
  let bodyHide = true;
  copyrightList[__Ox80931[0x12]]((_0xb7a8xa) => {
    if (_0xb7a8xa === copyright) {
      bodyHide = false;
    }
  });
  if (true === bodyHide) {
    document[__Ox80931[0x7]][__Ox80931[0x6]] = __Ox80931[0x8];
  }
  (function (
    _0xb7a8xb,
    _0xb7a8xc,
    _0xb7a8xd,
    _0xb7a8xe,
    _0xb7a8xf,
    _0xb7a8x10
  ) {
    _0xb7a8x10 = __Ox80931[0x13];
    _0xb7a8xe = function (_0xb7a8x11) {
      if (typeof alert !== _0xb7a8x10) {
        alert(_0xb7a8x11);
      }
      if (typeof console !== _0xb7a8x10) {
        console[__Ox80931[0x14]](_0xb7a8x11);
      }
    };
    _0xb7a8xd = function (_0xb7a8x12, _0xb7a8xb) {
      return _0xb7a8x12 + _0xb7a8xb;
    };
    _0xb7a8xf = _0xb7a8xd(
      __Ox80931[0x15],
      _0xb7a8xd(__Ox80931[0x16], __Ox80931[0x17])
    );
    try {
      _0xb7a8xb = __encode;
      if (
        !(
          typeof _0xb7a8xb !== _0xb7a8x10 &&
          _0xb7a8xb === _0xb7a8xd(__Ox80931[0x18], __Ox80931[0x19])
        )
      ) {
        _0xb7a8xe(_0xb7a8xf);
      }
    } catch (e) {
      _0xb7a8xe(_0xb7a8xf);
    }
  })({});
  document.writeln(
    '<script>let a = "%c  ' +
      copyright +
      '  %c \u0076\u0075\u0065\u002d\u0061\u0064\u006d\u0069\u006e\u002d\u0062\u0065\u0061\u0075\u0074\u0069\u0066\u0075\u006c";let b = "color: #fadfa3; background: #030307; padding:5px 0;";let c = "background: #fadfa3; padding:5px 0;";console.log(a, b, c)</script>'
  );
}
