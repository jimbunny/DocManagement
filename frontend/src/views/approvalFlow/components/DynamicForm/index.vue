...
import { debounce } from '@/utils/approval'
...
export default {
  ...
  methods: {
    ...
    handlerListChange(val){
      const vm = this
      this.$store.commit('approval/clearPCondition') // 清除所有条件 重新检测赋值
      const canUsedAsPCon = (conf, parent) => {
        ...
        const loop = (data, parent) => {
          ...
          if(Array.isArray(data)){
            ...
            ? vm.$store.commit("approval/addPCondition", data) 
            : vm.$store.commit("approval/delPCondition", data.formId)
          }
        }
        loop(this.drawingList)
        saveDrawingList(this.drawingList)
        this.$store.commit('approval/updateFormItemList', this.drawingList)
    ...
    empty() {
      ...
      this.$confirm("确定要清空所有组件吗？", "提示", { type: "warning" }).then(
        () => {
          this.drawingList = [];
          // this.idGlobal = 100;
          this.$store.commit('approval/clearPCondition')
        }
      );
    },
    ...
    checkColItem (cmp) {
      if(!cmp) return false
      const isPcon = this.$store.state.approval.processConditions.find(t => t.formId == cmp.formId) ? true : false
    ...
    drawingItemDelete(index, parent) {
      ...
      this.$store.commit("approval/delPCondition", parent[index].formId);
